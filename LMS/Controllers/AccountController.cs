...
namespace LMS.Controllers
{
  ...
  public class AccountController : CommonController
  {
    ...
    {
      // Retrieve uIDs from all 3 tables
      var query1 =
      ...
      // Union the query results and take the max value from the union (alphabetical comparison; no need to remove the 'u' or convert to int yet)
      //var max = query1.Union(query2).Union(query3).Max(s => s.uID);
      var max1 = query1.Max(s => s.uID);
      ...
      max = max == null ? "u0000000" : max;

      // Convert to int and add 1 for new uID value
      int newUIDValue = Int32.Parse(max.Substring(1)) + 1;

      // Re-add the 'u' and any zeros necessary for padding
      String newUID = "u" + newUIDValue.ToString().PadLeft(7, '0');
      ...
      {
        case "Student":
          Student student = new Student() { FName = fName, LName = lName, Dob = DOB, Abrv = SubjectAbbrev, UId = newUID };
          db.Student.Add(student);
          break;
        case "Professor":
          Professor professor = new Professor() { FName = fName, LName = lName, Dob = DOB, Abrv = SubjectAbbrev, UId = newUID };
          db.Professor.Add(professor);
          break;
        case "Administrator":
          Administrator administrator = new Administrator() { FName = fName, LName = lName, Dob = DOB, UId = newUID };
